...
(defmethod sql.tx/field-base-type->sql-type [:crossdata :type/Boolean]    [_ _] "BIT")
(defmethod sql.tx/field-base-type->sql-type [:crossdata :type/Date]       [_ _] "DATE")
(defmethod sql.tx/field-base-type->sql-type [:crossdata :type/DateTime]   [_ _] "DATETIME")
(defmethod sql.tx/field-base-type->sql-type [:crossdata :type/Decimal]    [_ _] "DECIMAL")
(defmethod sql.tx/field-base-type->sql-type [:crossdata :type/Float]      [_ _] "FLOAT")
...
(defmethod sql.tx/field-base-type->sql-type [:crossdata :type/Text]       [_ _] "VARCHAR(254)")
(defmethod sql.tx/field-base-type->sql-type [:crossdata :type/Time]       [_ _] "TIME")

(defmethod tx/dbdef->connection-details :crossdata [_ context {:keys [database-name]}]
  {:host     (tx/db-test-env-var-or-throw :crossdata :host)
   :port     (Integer/parseInt (tx/db-test-env-var-or-throw :crossdata :port "1433"))
   :user     (tx/db-test-env-var-or-throw :crossdata :user)
   :password (tx/db-test-env-var-or-throw :crossdata :password)
   :db       (when (= context :db)
               database-name)})

(defmethod sql.tx/drop-db-if-exists-sql :crossdata [_ {:keys [database-name]}]
...
(defmethod sql.tx/drop-table-if-exists-sql :crossdata [_ {:keys [database-name]} {:keys [table-name]}]
...
(defn- server-spec []
  (sql-jdbc.conn/connection-details->spec :crossdata
    (tx/dbdef->connection-details :crossdata :server nil)))
...
(defmethod tx/create-db! :crossdata [driver {:keys [database-name], :as db-def} & options]
...
(defmethod sql.tx/pk-sql-type :crossdata [_] "INT IDENTITY(1,1)")
