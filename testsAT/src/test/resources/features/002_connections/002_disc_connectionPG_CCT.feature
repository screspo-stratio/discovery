...
Feature: Connection on Postgres
  ...
  Scenario:[QATM-1866][02] Create database for Discovery on Postgres
  ...
  @skipOnEnv(SKIP_DATABASE_CREATION)
  Scenario:[QATM-1866][03] Create data for Discovery on Postgres
    Given I open a ssh connection to '!{pgIP}' with user '${CLI_USER:-root}' and password '${CLI_PASSWORD:-stratio}'
  ...
  Scenario:[04] Check Postgres database connection
  ...
  Scenario:[QATM-1866][05] Delete database for Discovery on Postgrestls
